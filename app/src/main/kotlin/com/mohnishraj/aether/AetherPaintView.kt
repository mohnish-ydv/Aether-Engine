...
class AetherPaintView(context: Context) : View(context) {
    ...
    init {
        ...
        contentDescription = context.getString(R.string.paint_preview_description)
    }

    fun submit(list: DisplayList) {
        displayList = list
        invalidate()
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val scale = min(width / list.viewport.width.toFloat(), height / list.viewport.height.toFloat()).coerceAtLeast(0.01f)
        val dx = (width - list.viewport.width.toFloat() * scale) / 2f
        canvas.withSave {
            withTranslation(dx, 0f) {
                ...
                for (command in list.commands) {
                    when (command) {
                        is PaintCommand.PushClip -> {
                            saveStack.addLast(save())
                            if (command.radii.isSquare) clipRect(command.rect.toRectF())
                            else clipPath(roundedPath(command.rect.toRectF(), command.radii))
                        }
                        is PaintCommand.PopClip -> if (saveStack.isNotEmpty()) restoreToCount(saveStack.removeLast())
                ...
                while (saveStack.isNotEmpty()) restoreToCount(saveStack.removeLast())
            }
        }
    }
    ...
    private fun com.mohnishraj.aether.core.layout.LayoutRect.toRectF(): RectF = RectF(x.toFloat(), y.toFloat(), right.toFloat(), bottom.toFloat())
